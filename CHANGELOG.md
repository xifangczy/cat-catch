...
[Fixed] 修复 firefox android下无法使用问题
...
[Updated] m3u8 解析器 增加 数据预处理 选项 (切片剔除图片数据)

[Updated] m3u8 解析器 从缓存中读取数据m3u8 解决"一次性url"问题

[Updated] aria2 RPC 支持填写下载目录

[Updated] 重构替换标签 现在支持标签嵌套 使用了函数的复杂标签可能与旧版预期不一致 请注意调整或反馈问题

[Updated] 增加俄语翻译 感谢来自北方的朋友 @Nikitamce

[Fixed] aria2 RPC 在firefox中无法提交http问题 #1000

[Fixed] 录制脚本 无src属性的视频无法读取问题

[Fixed] 深度搜索 某些情况下无法正确分析出密钥问题

[Fixed] m3u8 解析器 firefox 中 blob url 无法下载问题

[Fixed] 修复 `m3u8dl自定义协议`和`调用程序` 开启`确认参数`会造成多次调用问题
...
[Updated] 设置 过滤大小 现在支持使用表达式、范围、单位 例如 `>100 KB` `<1 GB` `=500 KB` `<=500 KB` `500-1000 MB`
